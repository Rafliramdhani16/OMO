<x-auth-layout>
    ...
    <div x-data="{ loading: false }" class="space-y-6">
        ...
        <form method="POST" action="{{ route('auth.register') }}" class="space-y-5 initial-animate stagger-1"
            @submit="loading = true">
            @csrf

            <div class="grid grid-cols-2 gap-4">
                <div>
                    <x-auth-input type="text" name="firstname" placeholder="Nama depan" icon="user"
                        :value="old('firstname')" required autocomplete="firstname" />
                </div>
                <div>
                    <x-auth-input type="text" name="lastname" placeholder="Nama belakang" icon="user"
                        :value="old('lastname')" required autocomplete="lastname" />
                </div>
            </div>

            <div>
                <x-auth-input type="text" name="username" placeholder="Username" icon="at-symbol"
                    :value="old('username')" required autocomplete="username" />
            </div>

            <div>
                <x-auth-input type="email" name="email" placeholder="Alamat email" icon="envelope" :value="old('email')"
                    required autocomplete="email" />
            </div>

            <div>
                <x-auth-input type="password" name="password" placeholder="Kata sandi" icon="lock" required
                    autocomplete="new-password" />
            </div>

            <div>
                <x-auth-input type="password" name="password2" placeholder="Konfirmasi kata sandi" icon="lock" required
                    autocomplete="new-password" />
            </div>
        ...
        <p class="text-center text-gray-600 initial-animate stagger-4">
            Sudah punya akun?
            <a href="{{ route('auth.login') }}" class="text-blue-500 relative group inline-block">
                <span
