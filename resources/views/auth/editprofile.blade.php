...
@section('content')
<div x-data="{ showUploadOption: false }">
    ...
    <div class="flex-1 lg:ml-64">
        <div class="max-w-6xl mx-auto p-4 lg:p-8">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                <form action="{{ route('auth.profile') }}" method="POST" enctype="multipart/form-data" @submit.prevent="$event.target.submit(); $dispatch('notification', {
                        type: 'success',
                    ...
                    <div class="p-6 border-b border-gray-100 flex justify-center">
                        <div class="relative group">
                            ...
                            <button @click.prevent="showUploadOption = !showUploadOption"
                            ...
                            <div x-show="showUploadOption" @click.away="showUploadOption = false"
                                ...
                                <label
                                    class="block px-4 py-2 hover:bg-gray-50 cursor-pointer transition-colors duration-200">
                                    <span>Unggah Foto</span>
                                    <input type="file" name="image" class="hidden" accept="image/*" @change="$dispatch('notification', {
                                            type: 'success',
                                            message: 'Foto berhasil diunggah'
                                        })">
                                </label>
                                <button type="button" @click="
                                    fetch('{{ route('auth.removephoto') }}', {
                                        method: 'PUT',
                                        headers: {
                                    ...
                                    .then(response => {
                                        if (response.ok) {
                                            $dispatch('notification', { type: 'success', message: 'Foto berhasil dihapus' });
                                            window.location.reload();
                                        } else {
                                            $dispatch('notification', { type: 'error', message: 'Gagal menghapus foto' });
                                            console.error('Gagal menghapus foto');
                                        }
                                    })
                                    .catch(error => {
                                        $dispatch('notification', { type: 'error', message: 'Terjadi kesalahan saat menghapus foto' });
                                        console.error('Error:', error);
                                    });
                                    showUploadOption = false;
                                "
                                    class="w-full text-left px-4 py-2 hover:bg-red-50 text-red-600 transition-colors duration-200">
                                    Hapus Foto
                                </button>
                            </div>
                        </div>
            ...
            <div class="flex justify-end mt-6 space-x-4">
                <button type="reset"
                    class="bg-gray-200 text-gray-700 px-6 py-3 rounded-lg hover:bg-gray-300 transition duration-200">Cancel</button>
                <button type="submit"
                    class="bg-blue-600 text-white px-6 py-3 rounded-lg hover:bg-blue-700 transition duration-200">Simpan</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    document.addEventListener('alpine:init', () => {
        Alpine.data('yourComponentName', () => ({ // You can name this component
            showUploadOption: false,
            removePhoto() {
                ...
                .then(response => {
                    if (response.ok) {
                        this.$dispatch('notification', { type: 'success', message: 'Foto berhasil dihapus' });
                        // Optionally reload the image or the whole page
                        window.location.reload();
                    } else {
                        this.$dispatch('notification', { type: 'error', message: 'Gagal menghapus foto' });
                        console.error('Gagal menghapus foto');
                    }
                })
                .catch(error => {
                    this.$dispatch('notification', { type: 'error', message: 'Terjadi kesalahan saat menghapus foto' });
                    console.error('Error:', error);
                });
