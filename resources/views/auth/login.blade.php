<x-auth-layout>
    ...
    <div x-data="{ loading: false }" class="space-y-6">
        ...
        <form method="POST" action="{{ route('auth.login') }}" class="space-y-5 initial-animate stagger-1" @submit="loading = true">
            @csrf

            <div>
                <x-auth-input type="email" name="email" placeholder="Masukkan email Anda" icon="envelope"
            ...
            <div class="flex items-center justify-between initial-animate stagger-2">
                <a href="{{ route('auth.forget') }}"
                    ...
                    <span class="inline-block transform transition-all duration-300 group-hover:scale-105 group-hover:text-blue-600">
                        Lupa Kata Sandi?
                    </span>
